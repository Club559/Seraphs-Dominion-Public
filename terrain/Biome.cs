...
namespace terrain
{
    internal class Biome
    {
        ...
        {
            var moisture = new Dictionary<MapNode, double>();
        ...
        {
            var sorted = new List<double>(nodes.Values.Distinct());
            sorted.Sort();
            ...
            var ret = new Dictionary<MapPolygon, double>();
        ...
        private void Randomize(TerrainTile[,] buff)
        {
            int w = buff.GetLength(0);
            int h = buff.GetLength(1);
            var tmp = (TerrainTile[,]) buff.Clone();
        ...
        private static void BlurElevation(TerrainTile[,] tiles, double radius)
        {
            ...
            int shift, source;
            var blurDiam = (int) Math.Pow(radius, 2);
        ...
        {
            var kernel = new double[gaussianWidth];
