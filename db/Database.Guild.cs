...
namespace db
{
    partial class Database
    {
        ...
        public List<GuildStruct> GetGuilds()
        {
            ...
            var guilds = new List<GuildStruct>();
            using (MySqlDataReader rdr = cmd.ExecuteReader())
            {
        ...
        public string HttpGetGuildMembers(int num, int offset, Account acc)
        {
            ...
            ret += "<Guild name=\"" + guild.Name + "\" id=\"" + guild.Id + "\">";
            ret += "<TotalFame>" + guild.TotalGuildFame + "</TotalFame>";
            ret += "<CurrentFame>" + guild.GuildFame + "</CurrentFame>";
            ret += "<HallType>Guild Hall " + guild.Level + "</HallType>";
