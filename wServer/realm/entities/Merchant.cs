...
namespace wServer.realm.entities
{
    public class Merchant : SellableObject
    {
        ...
        private const int BUY_NO_SOUL = 9;
        ...
        protected bool TryDeduct(Player player)
        {
            ...
            Manager.Data.AddPendingAction(db => db.ReadStats(acc));
            ...
            if (truePrice.Item2 == CurrencyType.Souls)
            {
                if (acc.Souls < truePrice.Item1) return false;
                Manager.Data.AddPendingAction(db =>
                {
                    player.Souls = acc.Credits = db.UpdateSouls(acc, -truePrice.Item1);
                    player.UpdateCount++;
                });
                return true;
            }
        ...
        protected bool TryAdd(Player player)
        {
            ...
            if (truePrice.Item2 == CurrencyType.Souls)
            {
                ...
                Manager.Data.AddPendingAction(db =>
                {
                    player.Souls = acc.Souls = db.UpdateSouls(acc, truePrice.Item1);
                    player.UpdateCount++;
                });
                return true;
            }
                ...
                if (TryDeduct(player))
                {
                    ...
                    for (int i = 0; i < player.Inventory.Length; i++)
                    {
                        ...
                        if (player.Inventory[i] == null &&
                            (player.SlotTypes[i] == 10 || player.SlotTypes[i] == Convert.ToInt16(ist.SlotType)))
